...
for CrIndex in range(len(Crs)):
    for hmaxIndex in range(N):
        ...
        V.SetOrder(TRIG,3)
        V.Update()
        W = VectorH1(mesh, order=4, dirichlet="rect")
        Q = H1(mesh, order=1)
        R = NumberSpace(mesh)
        X = V*Q*R
        ...
        vtk = VTKOutput(ma=mesh,
                        ...
                        filename="./MINIFigures/MINI_US_{}_{}".format(str(Cr),str(hmax)),
                        subdivision=3)
        vtk.Do()
        sleep(3)
...
np.save("MINI_US_ErrorPressure",ErrorPressure) #Error is measured in L2
np.save("MINI_US_ErrorVelocity",ErrorVelocity) #Error is measured in H1 
